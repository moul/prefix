...
generate: install
	...
	echo '$$> prefix -h' > .tmp/usage.txt
	prefix -h 2>> .tmp/usage.txt

	echo '$$> generate-fake-data | prefix' > .tmp/default.txt
	generate-fake-data --seed=42 --lines=10 --dict=lorem-ipsum --no-stderr | prefix >> .tmp/default.txt

	echo '$$> generate-fake-data | prefix -format="#{{.LineNumber3}} {{.Uptime}} {{.Duration}} | "' > .tmp/example-1.txt
	...
	echo '$$> generate-fake-data | prefix -format="{{.LineNumber3}} "' > .tmp/example-2.txt
	generate-fake-data --seed=42 --lines=10 --dict=lorem-ipsum --no-stderr --sleep-max=0 | prefix --format="{{.LineNumber3}} " >> .tmp/example-2.txt

	echo '$$> generate-fake-data | prefix -format=">>> "' > .tmp/example-3.txt
